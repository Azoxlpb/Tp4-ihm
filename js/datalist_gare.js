...
async function addStation(){
    ...
    let value =  document.querySelector(`search_depart`).value;
    console.log(value);
    for (let i = 0; i < req.length; i++) {
		if (req[i].city == value) {
			document.querySelector(`#search_depart`).innerHTML += `<option value="${req[i].name}"></option>`;
		}
	}
...
let req2 = await (await fetch(`https://gigondas.iut-valence.fr:1112/sprietna/ihm/tp4/stations`)).json();
	document.querySelector(`#search_depart_gare`).innerHTML = '';
    let value2 =  document.querySelector(`#search_depart`).value;
    for (let i = 0; i < req2.length; i++) {
		if (req2[i].city == value2) {
			document.querySelector(`#search_depart`).innerHTML += `<option value="${req2[i].name}"></option>`;
		}
	}
*/